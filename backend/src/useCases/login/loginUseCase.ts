...
export default class LoginUseCase{
    ...
    async execute({email,password}:LoginDTO):Promise<LoginResponseDTO>{
        ...
        if(!user){throw new AppError(HtppStatus.UNAUTHORIZED, "E-mail or password incorrects")}
        const passwordMatch = compareSync(password, user.password)
        if(!passwordMatch){throw new AppError(HtppStatus.UNAUTHORIZED, "E-mail or password incorrects")}
